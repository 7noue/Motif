<script lang="ts">
    import { X, Play, Heart, Bookmark, Brain, Gauge, Star } from 'lucide-svelte';
    ...
    let tags = $state<Tag[]>(JSON.parse(JSON.stringify(movie.initialTags || [])));
    ...
    const voteTag = (index: number) => {
        ...
        // In Svelte 5 we can mutate state directly if it's an array inside $state, 
        // but for deep objects inside arrays, it's safer to clone or use fine-grained reactivity.
        // Here we'll just mutate the specific tag since `tags` is a proxy.
...
>
    ...
    >
        ...
        <button onclick={close} class="absolute top-4 right-4 z-50 p-2 bg-black/40 backdrop-blur-md rounded-full text-white/70 hover:text-white border border-white/10 hover:bg-white/10 transition-colors cursor-pointer"><X class="w-4 h-4" /></button>
            ...
            <div class="p-5 flex flex-col gap-5 bg-[#0e0e0e] shrink-0">
                ...
                <div class="grid grid-cols-3 gap-2 py-2 border-b border-white/5">
                    ...
                    <div class="flex flex-col items-center border-l border-white/5">
                        ...
                        <span class="text-yellow-500 font-bold text-lg flex items-center gap-1"><Star class="w-3 h-3 fill-current" /> {Math.round(movie.popularity || 0)/10}</span>
                    </div>
                    <div class="flex flex-col items-center border-l border-white/5 text-center px-1">
                        ...
                        <div class="flex flex-col items-center leading-none">
                            ...
                            <span class="text-[7px] text-neutral-500 uppercase tracking-tight mt-0.5 w-full truncate px-1" title={movie.certData?.reason}>{movie.certData?.reason || 'Not Rated'}</span>
                        </div>
                    </div>
                </div>
        ...
        <div class="w-full md:w-7/12 flex flex-col bg-[#0e0e0e]">
            <div class="flex-1 overflow-y-auto custom-scrollbar p-6 md:p-8 flex flex-col gap-6">
                ...
                <div class="border-l-2 border-indigo-500 pl-4 py-1">
                     <p class="text-lg text-white/90 font-light italic leading-snug">"{movie.momentSentence}"</p>
                </div>

                <div>
                     <h3 class="text-[10px] font-bold text-neutral-500 uppercase tracking-widest mb-2">Story</h3>
                     <p class="text-sm text-neutral-400 font-light leading-relaxed">{movie.overview}</p>
                     <div class="mt-4 pt-3 border-t border-white/5 flex flex-col gap-1.5 text-xs text-neutral-400">
                         <p><span class="text-white font-bold uppercase text-[10px] tracking-wider mr-2">Director</span> {movie.director}</p>
                         <p><span class="text-white font-bold uppercase text-[10px] tracking-wider mr-2">Cast</span> {movie.cast?.join(", ")}</p>
                         <p><span class="text-white font-bold uppercase text-[10px] tracking-wider mr-2">Language</span> {movie.original_language}</p>
                     </div>
                </div>
                ...
                <div class="pt-2">
                    ...
                    <div class="flex flex-wrap gap-2">
                        {#each tags as tag, i}
                            <button onclick={() => voteTag(i)} class="group relative flex items-center gap-2 px-3 py-1.5 rounded-md border text-xs font-medium transition-all active:scale-95 select-none cursor-pointer {tag.userVoted ? 'bg-indigo-500/10 border-indigo-500/40 text-indigo-300 shadow-[0_0_10px_rgba(99,102,241,0.2)]' : 'bg-[#141414] border-white/5 text-neutral-400 hover:border-neutral-700 hover:text-neutral-300'}">
                                <span>{tag.name}</span>
                        ...
                        {#if customTagsAdded < MAX_CUSTOM_TAGS}
                            {#if !showAddTagInput}
                                <button onclick={() => showAddTagInput = true} class="px-2.5 py-1.5 rounded-md border border-dashed border-white/10 text-neutral-600 text-xs hover:border-white/20 hover:text-neutral-400 transition-all cursor-pointer">+</button>
                            {:else}
                                <form onsubmit={(e) => { e.preventDefault(); addNewTag(); }} class="flex items-center">
                                    <input use:focusOnMount bind:value={newTagValue} class="bg-transparent border-b border-indigo-500 text-white text-xs w-20 outline-none px-1 py-1" placeholder="Add..." onblur={() => {if(!newTagValue) showAddTagInput=false}} />
                                </form>
                            {/if}
                        {/if}
                    </div>
                        ...
                        {#each movie.recommendations as rec}
                            <div class="min-w-[120px] w-[120px] group cursor-pointer">
                                <div class="aspect-[2/3] rounded-lg overflow-hidden bg-neutral-900 border border-white/5 relative mb-2">
                                    <div class="absolute inset-0 bg-linear-to-br {rec.gradient || 'from-gray-700 to-gray-900'} opacity-40 group-hover:opacity-60 transition-all"></div>
            ...
            <div class="p-6 border-t border-white/5 bg-[#0e0e0e] flex items-center gap-3 z-10 mt-auto">
                ...
                <button onclick={() => toggleInteraction('heart')} class="w-12 h-12 rounded-xl border border-white/10 bg-white/5 text-neutral-300 flex items-center justify-center hover:bg-white/10 transition-all active:scale-95 cursor-pointer {isHearted ? 'text-rose-400 border-rose-500/30 bg-rose-500/10' : ''}">
                    <Heart class="w-5 h-5 {isHearted ? 'fill-current' : ''}" />
                </button>

                <button onclick={() => toggleInteraction('bookmark')} class="w-12 h-12 rounded-xl border border-white/10 bg-white/5 text-neutral-300 flex items-center justify-center hover:bg-white/10 transition-all active:scale-95 cursor-pointer {isBookmarked ? 'text-emerald-400 border-emerald-500/30 bg-emerald-500/10' : ''}">
