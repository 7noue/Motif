<script lang="ts">
    ...
    import { currentUser, isLoginModalOpen, toast } from '$lib/stores';
    ...
    async function handleGoogleLogin() {
        try {
            ...
            // If successful, the auth listener in stores.ts handles the rest
            close(); 
        } catch (error) {
            console.error(error);
            // Error toast is already handled in store, but we keep modal open if it fails
        }
    }
</script>
...
>
    ...
    >
        <button 
            onclick={close}
            class="absolute top-4 right-4 p-2 text-neutral-500 hover:text-white transition-colors rounded-full hover:bg-white/5"
        >
            <X class="w-4 h-4" />
        </button>
        ...
        <div class="w-full space-y-3">
            <button 
                ...
                class="w-full h-12 rounded-xl bg-white text-black font-bold text-sm flex items-center justify-center gap-3 hover:bg-neutral-200 transition-colors group"
            >
        ...
        <p class="mt-6 text-[10px] text-neutral-600">
            By continuing, you agree to our <span class="text-neutral-400 underline decoration-neutral-600 underline-offset-2">Thesis on Taste</span>.
        </p>
    </div>
</div>