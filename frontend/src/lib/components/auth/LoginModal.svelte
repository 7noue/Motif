<script lang="ts">
    ...
    function close() {
        if (isLoggingIn) return; // Prevent closing while loading
    ...
    async function handleGoogleLogin() {
        ...
        try {
            ...
            // Success! The store listener will handle the UI update.
            // We just close the modal.
            toast.show("Welcome to the ecosystem.", "success");
            $isLoginModalOpen = false;
        } catch (error) {
...
>
    ...
    >
        ...
        <button 
            ...
            class="w-full h-12 rounded-xl bg-white text-black font-bold text-sm flex items-center justify-center gap-3 hover:bg-neutral-200 transition-colors group cursor-pointer disabled:opacity-70 disabled:cursor-wait"
        >
            {#if isLoggingIn}
